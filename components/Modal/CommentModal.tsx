        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            try {
                const sessionResponse = await fetch('/api/comments/sendComment', {
                    ...
                    body: JSON.stringify({
                        FromWho: currentUserId,
                        tweetsId: uniqueTweetId,
                        parent: post.attributes.tweetsId,
                        tweet: formData.body,
                    }),
                });
            ...
            <div className="fixed inset-0 z-10 flex items-start justify-center bg-white bg-opacity-30">
                <div className="bg-black text-white p-4 rounded-lg max-w-lg w-full">
                    <div className="flex justify-between items-center">
                        <button onClick={closeModal} className="text-gray-400 hover:text-gray-200">&times;</button>
                    </div>
                    <div className="cursor-pointer">
                        <div className='flex'>
                            <Avatar
                                ownersId={post.attributes.ownersId}
                            />
                            <div className='w-full'>
                                <div className='flex'>
                                    <p className='text-white mt-1 font-semibold'>{user.username}</p>
                                    <p className='text-gray-500 mt-1 pl-1'>@{user.nickname}</p>
                                ...
                                <p className='text-gray-500 mt-3 pl-1'>
                                    <span className='text-blue-400'>@{user.username}</span> adlı kullanıcıya yanıt olarak
                                </p>
                            </div>
                        </div>
